...
$teamPhoto = get_field('team_photo');
      ...
      //var_dump($teamPhoto);
      //print '</pre>';
      print '<img src="'.$teamPhoto["sizes"]["content-banner"].'">';
      //var_dump( $teamPhoto);
?>
  <div class="cf">
    <div class="main-col">
...
*/ ?>
      ...
      <?php if (count($posts) > 0): $n = 0; ?>
   	 <div class="group event-sponsor">
          <h3>Other Events</h3><ul class="event-locations">
  ...
  <div class="inner-section" id="projects">
    <h2>Projects</h2>
  ...
  <?php } ?>
   <?php include( get_template_directory() . '/partials/photo-gallery.php'); ?>
      <?php if (!empty($main_sponsor_logo)) { ?>
        <div class="group event-sponsor">
        <?php
	  $src = $main_sponsor_logo['sizes']['large'];
          if (!empty($main_sponsor_url)) {
            if($main_sponsor_url_check !== 'http'){
              echo '<a href="http://' . $main_sponsor_url . '" target="_blank">';
            } else {
